...
自联邦学习范式提出以来，已被用于开发各种GANs，这些方法的细微差别在于是否聚合生成器、判别器或两者。然而，对于这些生成模型来说，高通信成本源于反复传输模型参数。为规避这一问题，另一些改进方法**提出设置一个中央生成器并使用本地判别器的损失值来更新生成器，与传输整个模型相比，服务器与本地客户端之间的数据传输量大大减少**。然而，**这些方案并未专门针对非独立同分布（non-i.i.d）数据，缺乏在实际应用中的扎实基础**。上述方法适用于单一数据源，但在多源异构数据源上则不适用。此外，现有针对多源数据的生成模型采用集中模式，限制了它们在分布式数据孤岛中的适用性。
...
​	这种方式下，$G_k$在从所有本地判别器收集完损失函数后再更新。不同于传统的集中平均聚合的方法，本文的同步集成可以用于本地分布非iid的情况。集成的损失由下式计算：
$$
\mathcal{L}_{D_{sync}}(G_{k})=\sum_{j\in\mathbb{J}_{k}}\frac{e^{\mathcal{L}_{D_{kj}}(G_{k})}}{\sum_{j'\in\mathbb{J}_{k}}e^{\mathcal{L}_{D_{kj}}(G_{k})}}\mathcal{L}_{D_{kj}}(G_{k}).
$$


​	式中，$\mathcal{L}_{D_{kj}}(G_k)$值越大，则$D_{kj}$认为生成的数据越真实，因此就分配一个更大的权重去聚合。这种指数加权的聚合方式在一个2019年的工作中被验证过是有效的。在获得该集成损失后，$G_k$就可以梯度下降的方式更新。

### 异步更新

​	由于IoT设备可能有不同容量、计算效率和传输能力，同步方式可能会增加时间耗时。在异步方式中，$G_k$只要接收到一个本地判别器传来的损失就更新，而不用等到所有判别器都算完。对于$G_k$，过时度（staleness）越小的损失对准确更新的作用越大。考虑此，定义$\mathcal{L}_{D_{kj}}(G_k)$的staleness为$s_{kj}=T^{rec}_{kj}-T^{lat}_k$，前一个T是$G_k$接收到损失函数值的时间，后一个是$G_k$最近更新的时间。异步更新损失函数值如下式所示：

$$
\mathcal{L}_{D_{async}}(G_k)=\frac{1}{|\mathbb{M}_k|}\sum_{j\in\mathbb{M}_k}e^{-s_{kj}}\mathcal{L}_{D_{kj}}(G_k),
$$


​	其中$\mathbb{M}$是损失函数值同时到达$G_k$的设备数。式中，过时度越小就会被分配更大的权重来更新$G_k$。
...
​	在这种情况下，全局生成器希望在不同域条件下生成多域数据分布，每个区域的条件定义为$c_k$，表明了生成的数据的来源。因此GAN就使用到了cGAN；具体来说，就是$G_k$用隐藏变量z和域条件$c_k$来生成。同样的，生成器生成两批数据，$G_k(z_d,c_k)$和$G_k(z_g,c_k)$分别更新判别器和计算损失。

## 判别器

​	 同样包括更新和损失计算两个操作，与上述基本一致，只是公式的G里做了个条件$c_k$。
